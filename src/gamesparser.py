...
def analyze_line(line: str) -> str:
    ...
    if battle_started_pat.match(line):
        ...
        if match:
            ...
            sent_outs = set(filter(lambda x: sent_out_pat.match(x), log_arr_reference))
...
# the return type is tuple(two players playing the game, the rest of the log)
def create_log(log_arr: list[str]) -> list[str]:
    global log_arr_reference
    log_arr_reference = log_arr
    out = []
